...
var SearchEverything = ( function( $ ) {
	var r = {
			...
			handleWindowResize: function() {
				$( window ).on( 'resize', r.resizeSearchInput );
			},
			handleMetaboxActions: function() {
				...
				$( '.postbox h3, .postbox .handlediv' ).on(
					'click',
					function() {
						...
						setTimeout(
							...
							1
						); // Delay till WP finishes its own thing and then we can kick in.
					}
				);
			},
				...
				$.ajax(
					{
						...
						success: function( data ) {
							...
							if ( ! window.externalSearchEnabled ) {
								ownResults.before(
									'<div id="se-metabox-own-powersearch" class="se-metabox-results-list"><h4>Results from around the web</h4><p>If you want to use external search, you need to enable it in your <a class="se-settings-link" href="options-general.php?page=extend_search" target="_blank"><strong>settings</strong></strong></a>.</p></div>' );
								$( '#se-metabox-own-powersearch' ).show();
							} else if ( data.external.length === 0 ) {
							...
							if ( data.own.length === 0 ) {
								$( '#se-metabox-results' ).append(
									'<p class="se-no-results">It seems we haven\'t found any results for search term <strong>' + input.prop(
										'value' ) + ' on your blog</strong>.</p>' );
						...
						error: function() {
							$( '.se-spinner, .se-no-results' ).remove();
							$( '#se-metabox-results' ).append(
								'<p class="se-no-results">There was something wrong with the search. Please try again. If this happens a lot, please check out our <a href="https://wordpress.org/support/plugin/search-everything" target="_blank">Support forum</a>.</p>' );
						},
					}
				);
			...
			initResultBehaviour: function() {
					...
					function( ev ) {
						...
						if ( inserted.length ) {
							...
							if ( window.send_to_editor ) {
								window.send_to_editor( insertHtml.html() );
							} else {
							// Dunno yet.
							}
						}
				...
				metabox.on(
					'click',
					'#se-metabox-external-results li',
					function( ev ) {
						var insertHtml = $( html ),
							listItem = $( this ),
							date = ( function() {
